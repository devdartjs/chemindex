...
const getReagents = async (req, res) => {
  try {
    ...
    if (Number.isNaN(page) || Number.isNaN(limit)) return res.status(400).json({
    ...
    if (page > 100 || limit > 100) return res.status(400).json({
      message: "Page or limit value exceeds maximum of 100."
    });
    const startIndex = (page - 1) * limit;
    const totalReagents = await Reagent.countDocuments({
      createdBy: userId
    });
    ...
    if (!reagents || reagents.length === 0) return res.status(404).json({
      message: "No reagents found on this page."
    });
