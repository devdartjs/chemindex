...
const createReagent = async (req, res) => {
  try {
    ...
    if (!reagents) return res.status(400).json({
      message: "There is no reagents here!"
    });
    if (reagents.length >= 16) {
      return res.status(403).json({
        message: "To create more reagents, you need to update your account. Please fill out the waiting list form.",
    ...
    if (!casNumber) return res.status(400).json({
      message: "Invalid casNumber"
    });
    const existingReagent = await Reagent.findOne({
      casNumber: casNumber.trim(),
      createdBy: userId
    });
    if (existingReagent) return res.status(400).json({
